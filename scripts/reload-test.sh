...
"$BINARY" strict-single curl 500kb 2>/dev/null
sleep 1
...
"$BINARY" strict-single curl 100kb 2>/dev/null
sleep 0.5
"$BINARY" unstrict curl 2>/dev/null
sleep 0.5
...
for rate in 100kb 500kb 1mb 100kb; do
    "$BINARY" strict-single curl "$rate" 2>/dev/null || ERRORS=$((ERRORS + 1))
    sleep 0.3
done
if [ "$ERRORS" -eq 0 ]; then
...
"$BINARY" strict-single curl 10kb 2>/dev/null  # Very low rate → lots of drops
# Generate traffic
for i in 1 2 3; do
    curl -s -o /dev/null http://example.com/largefile 2>/dev/null &
done
...
DROPS_BEFORE=$("$BINARY" status 2>/dev/null | grep curl | awk '{print $5}' | head -1)
# Change rate
"$BINARY" strict-single curl 500kb 2>/dev/null
sleep 1
DROPS_AFTER=$("$BINARY" status 2>/dev/null | grep curl | awk '{print $5}' | head -1)
